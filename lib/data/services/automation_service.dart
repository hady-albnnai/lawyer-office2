...
class AutomationService {
  ...
  Future<void> _autoRecurringCourtSessions(
    ...
    final recurringSessions = await db.customSelect('''
      ...
      LIMIT 10
    ''', variables: [
      Variable.withDateTime(currentDate.subtract(const Duration(days: 90))),
    ]).get();

    for (final row in recurringSessions) {
      final data = row.data;
      final lastSessionDate = _asDate(data['session_date']);
      ...
      if (lastSessionDate != null) {
        ...
        if (nextSessionDate.isBefore(currentDate.add(const Duration(days: 7)))) {
          await taskDao.insertTask(DailyTasksCompanion(
            ...
            notes: const Value('ترحيل تلقائي من جلسة سابقة'),
  ...
  Future<void> _autoContractReminders(
    ...
    ''', variables: [
      ...
      Variable.withDateTime(currentDate.add(const Duration(days: 30))),
    ]).get();
    ...
    for (final row in expiringContracts) {
      ...
      if (expiryDate != null) {
        // التحقق من عدم وجود تذكير مسبق لنفس العقد
        final existingReminder = await db.customSelect(
          'SELECT id FROM daily_tasks WHERE source_type = ? AND source_id = ? AND task_type = ?',
          variables: [
        ...
        if (existingReminder.isNotEmpty) continue; // تذكير موجود مسبقاً
  ...
  Future<void> _autoCompanyPhases(
    ...
    final activeCompanies = await db.customSelect('''
      SELECT c.id AS id, c.name AS name, c.current_phase AS current_phase,
             MIN(p.scheduled_date) AS phase_completion_date
      FROM companies c
      LEFT JOIN company_phases p
             ON p.company_id = c.id AND p.status = 0
      WHERE c.legal_status = 'active'
        AND c.is_archived = 0
      GROUP BY c.id
      HAVING phase_completion_date IS NOT NULL
         AND phase_completion_date <= ?
    ''', variables: [
      Variable.withDateTime(currentDate.add(const Duration(days: 7))),
    ]).get();

    for (final row in activeCompanies) {
      final data = row.data;
      final companyId = data['id'] as int;
      final phaseCompletionDate = _asDate(data['phase_completion_date']);
      
      if (phaseCompletionDate != null) {
        // التحقق من عدم وجود مهمة مسبقاً لنفس الشركة
        final existingTask = await db.customSelect(
          'SELECT id FROM daily_tasks WHERE source_type = ? AND source_id = ? AND task_type = ?',
          variables: [
        ...
        if (existingTask.isNotEmpty) continue;
  ...
  Future<List<Map<String, dynamic>>> suggestAppointments({
    ...
    final busyDays = await db.customSelect('''
      ...
      LIMIT 5
    ''', variables: [
      Variable.withDateTime(targetDate),
      Variable.withDateTime(targetDate.add(const Duration(days: 30))),
    ]).get();
  ...
  Future<void> conditionalReschedule({
    ...
    ''', variables: [
      Variable.withInt(currentTaskId),
    ]).getSingle();
  ...
  Future<void> smartRescheduleBasedOnDecision({
    ...
    if (caseId != null && sessionDate != null) {
      ...
      ''', variables: [
        ...
        Variable.withDateTime(nextSessionDate),
      ]);
